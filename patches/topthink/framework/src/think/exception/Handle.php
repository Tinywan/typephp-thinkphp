...
class Handle
{
    ...
    public function report(Throwable $exception): void
    {
        if (!$this->isIgnoreReport($exception)) {
            ...
            if ($this->app->make('config')->get('log.record_trace')) {
                $log .= PHP_EOL . $exception->getTraceAsString();
            }

            try {
                $this->app->make('log')->record($log, 'error');
            } catch (Exception $e) {
            }
        }
    }
    ...
    protected function renderHttpException(Request $request, HttpException $e): Response
    {
        ...
        $template = $this->app->make('config')->get('app.http_exception_template');
    ...
    protected function getDeployMsg(Throwable $exception): array
    {
        ...
        if ($showErrorMsg || $this->app->make('config')->get('app.show_error_msg', false)) {
            $message = $this->getMessage($exception);
        } else {
            // 不显示详细错误信息
            $message = $this->app->make('config')->get('app.error_message');
        }
    ...
    protected function getDebugMsg(Throwable $exception): array
    {
        ...
        return [
            ...
            'tables'  => [
                'GET Data'            => $this->app->make('request')->get(),
                'POST Data'           => $this->app->make('request')->post(),
                'Files'               => $this->app->make('request')->file(),
                'Cookies'             => $this->app->make('request')->cookie(),
                'Session'             => $this->app->exists('session') ? $this->app->make('session')->all() : [],
                'Server/Request Data' => $this->app->make('request')->server(),
            ],
        ];
    }
