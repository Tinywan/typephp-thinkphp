...
class App extends Container
{
    ...
    public function initialize()
    {
        ...
        $this->envName = $this->envName ?: (string) $this->make('env')->get('env_name', '');
        $this->loadEnv($this->envName);

        $this->configExt = $this->make('env')->get('config_ext', '.php');
        ...
        // 监听AppInit
        $this->make('event')->trigger(AppInit::class);

        date_default_timezone_set($this->make('config')->get('app.default_timezone', 'Asia/Shanghai'));
    ...
    {
        // 加载默认语言包
        $langSet = $this->make('lang')->defaultLangSet();
        $this->make('lang')->switchLangSet($langSet);
    }
    ...
    protected function load(): void
    {
        ...
        if (is_file($this->runtimePath . 'config.php')) {
            $this->make('config')->set(include $this->runtimePath . 'config.php');
        } else {
            $this->loadConfig();
        }
    ...
    public function loadConfig()
    {
        ...
        foreach ($files as $file) {
            $this->make('config')->load($file, pathinfo($file, PATHINFO_FILENAME));
        }
    }
    ...
    protected function debugModeInit(): void
    {
        // 应用调试模式
        if (!$this->appDebug) {
            $this->appDebug = $this->make('env')->get('app_debug') ? true : false;
        }
    ...
    {
        if (isset($event['bind'])) {
            $this->make('event')->bind($event['bind']);
        }

        if (isset($event['listen'])) {
            $this->make('event')->listenEvents($event['listen']);
        }

        if (isset($event['subscribe'])) {
            $this->make('event')->subscribe($event['subscribe']);
        }      
    }
