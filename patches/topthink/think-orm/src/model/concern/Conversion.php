    ...
    */
    public function toArray(): array
    {
        ...
        foreach (['visible', 'hidden', 'append'] as $convert) {
            $convertData[$convert] = $this->getOption($convert);
            foreach ($convertData[$convert] as $key => $val) {
                if (is_string($key)) {
                    $relation[$key][$convert] = $val;
                    unset($convertData[$convert][$key]);
                } elseif (str_contains($val, '.')) {
                    [$relName, $name] = explode('.', $val);
                    $relation[$relName][$convert][] = $name;
                    unset($convertData[$convert][$key]);
                } elseif ($item = array_search($val, $mapping)) {
                    $convertData[$convert][$key] = $item;
                }
            }
        }
        $data  = $this->getData();
        $allow = array_diff(
            $convertData['visible'] ?: array_keys($data),
            $convertData['hidden']
        );
        ...
        foreach ($data as $name => $val) {
            if ($val instanceof Modelable || $val instanceof Collection) {
                if (in_array($name, $convertData['hidden'])) {
                    // 隐藏关联属性
                    unset($item[$name]);
                    continue;
                }
