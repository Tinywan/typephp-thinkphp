        ...
        try {
            ...
            if (null !== $callable) {
                if ($callable instanceof \Closure) {
                    ...
                    $ra = [$callable, '__invoke'];
                } elseif (\is_array($callable)) {
                    $ra = $callable;
                } elseif (false !== $i = strpos($callable, '::')) {
                    $ra = [substr($callable, 0, $i), substr($callable, 2 + $i)];
                } else {
                    $r = new \ReflectionFunction($callable);
                }
            } elseif (0 < $i = strpos($identifier, '::') ?: strpos($identifier, '->')) {
                $ra = [substr($identifier, 0, $i), substr($identifier, 2 + $i)];
            } else {
            ...
            if (null !== $ra) {
                try {
                    $r = new \ReflectionMethod($ra[0], $ra[1]);
                } catch (\ReflectionException $e2) {
                    $r = new \ReflectionClass($ra[0]);
                }
            }
            ...
            if (null !== $callable && $r instanceof \ReflectionFunctionAbstract) {
                ...
                $sig = ReflectionCaster::getSignature($s);

                if (str_ends_with($identifier, '()')) {
                    $this->value = substr_replace($identifier, $sig, -2);
                } else {
                    $this->value .= $sig;
                }
            }
