...
{
    public const UNSET_CLOSURE_FILE_INFO = ['Closure' => 'Symfony\Component\VarDumper\Caster\ReflectionCaster::unsetClosureFileInfo'];
    ...
    public static function castClosure(\Closure $c, array $a, Stub $stub, bool $isNested, int $filter = 0): array
    {
        ...
        $rc = new \ReflectionFunction($c);

        $a = static::castFunctionAbstract($rc, $a, $stub, $isNested, $filter);

        if (!$rc->isAnonymous()) {
            $stub->class = isset($a[$prefix.'class']) ? $a[$prefix.'class']->value.'::'.$rc->name : $rc->name;
        ...
        if ($f = $rc->getFileName()) {
            $stub->attr['file'] = $f;
            $stub->attr['line'] = $rc->getStartLine();
        }
        ...
        if ($f) {
            $a[$prefix.'file'] = new LinkStub($f, $rc->getStartLine());
            $a[$prefix.'line'] = $rc->getStartLine().' to '.$rc->getEndLine();
        }
    ...
    public static function castFunctionAbstract(\ReflectionFunctionAbstract $c, array $a, Stub $stub, bool $isNested, int $filter = 0): array
    {
        ...
        if (!($filter & Caster::EXCLUDE_VERBOSE) && $staticVars = $c->getStaticVariables()) {
            foreach ($staticVars as $sk => &$sv) {
                if (\is_object($sv)) {
                    $a[$prefix.'use']['$'.$sk] = new CutStub($sv);
                } else {
                    $a[$prefix.'use']['$'.$sk] = &$sv;
                }
            }
