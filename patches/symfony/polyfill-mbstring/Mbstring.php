...
final class Mbstring
{
    ...
    public static function mb_convert_variables($toEncoding, $fromEncoding, array &$vars = [])
    {
        $ok = true;
        foreach ($vars as &$item) {
            if (is_array($item)) {
                foreach ($item as &$inner) {
                    $inner = self::mb_convert_encoding($inner, $toEncoding, $fromEncoding);
                    if (false === $inner) {
                        $ok = false;
                    }
                }
                ...
                $item = self::mb_convert_encoding($item, $toEncoding, $fromEncoding);
                if (false === $item) {
                    $ok = false;
                }
            }
