...
class Filesystem implements FilesystemOperator
{
    ...
    {
        $moveConfig = $this->resolveConfigForMoveAndCopy($config);
        $from = $this->pathNormalizer->normalizePath($source);
        $to = $this->pathNormalizer->normalizePath($destination);

        if ($from === $to) {
            $resolutionStrategy = $moveConfig->get(Config::OPTION_MOVE_IDENTICAL_PATH, ResolveIdenticalPathConflict::TRY);

            if ($resolutionStrategy === ResolveIdenticalPathConflict::FAIL) {
                $e = UnableToMoveFile::sourceAndDestinationAreTheSame($source, $destination);
                throw $e;
            }
        ...
        $this->adapter->move($from, $to, $moveConfig);
    }

    public function copy(string $source, string $destination, array $config = []): void
    {
        $copyConfig = $this->resolveConfigForMoveAndCopy($config);
        $from = $this->pathNormalizer->normalizePath($source);
        $to = $this->pathNormalizer->normalizePath($destination);

        if ($from === $to) {
            $resolutionStrategy = $copyConfig->get(Config::OPTION_COPY_IDENTICAL_PATH, ResolveIdenticalPathConflict::TRY);

            if ($resolutionStrategy === ResolveIdenticalPathConflict::FAIL) {
                $e = UnableToCopyFile::sourceAndDestinationAreTheSame($source, $destination);
                throw $e;
            }
            if ($resolutionStrategy === ResolveIdenticalPathConflict::IGNORE) {
                return;
        ...
        $this->adapter->copy($from, $to, $copyConfig);
    }
    ...
    public function mimeType(string $path): string
    {
        ...
        if ($attributes->mimeType() === null) {
            $e = UnableToRetrieveMetadata::mimeType($path);
            throw $e;
        }
    ...
    {
        $generator = $this->resolvePublicUrlGenerator();
        if ($generator === null) {
            $e = UnableToGeneratePublicUrl::noGeneratorConfigured($path);
            throw $e;
        }
        $this->publicUrlGenerator = $generator;
        $urlConfig = $this->config->extend($config);
    ...
    public function temporaryUrl(string $path, DateTimeInterface $expiresAt, array $config = []): string
    {
        ...
        $e = UnableToGenerateTemporaryUrl::noGeneratorConfigured($path);
        throw $e;
    }

    public function checksum(string $path, array $config = []): string
    {
        $checksumConfig = $this->config->extend($config);

        if ( ! $this->adapter instanceof ChecksumProvider) {
            return $this->calculateChecksumFromStream($path, $checksumConfig);
        }
