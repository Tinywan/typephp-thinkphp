...
class Log extends Manager implements LoggerInterface
{
    ...
    {
        if (!is_null($name)) {
            return $this->app->make('config')->get('log.' . $name, $default);
        }

        return $this->app->make('config')->get('log');
    }
    ...
    public function createDriver(string $name)
    {
        ...
        return new Channel($name, $driver, $allow, $lazy, $this->app->make('event'));
    }
    ...
    {
        return $this->app->make('event')->listen(LogWrite::class, $listener);
    }
