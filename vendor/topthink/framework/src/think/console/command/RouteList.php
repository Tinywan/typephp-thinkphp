...
class RouteList extends Command
{
    ...
    protected function scanRoute($path, $root, $autoGroup)
    {
        ...
        foreach ($iterator as $fileinfo) {
            ...
            if ($fileinfo->getType() == 'file' && $fileinfo->getExtension() == 'php') {
                ...
                if ($groupName) {
                    $this->app->make('route')->group($groupName, function()  use ($fileinfo) {
    ...
    {
        $this->app->make('route')->clear();
        $this->app->make('route')->lazy(false);
        $autoGroup = $this->app->make('route')->config('route_auto_group');
        ...
        $routeList = $this->app->make('route')->getRuleList();
        $rows      = [];
