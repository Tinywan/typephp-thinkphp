...
class CliDumper extends AbstractDumper
{
    ...
    public function dumpString(Cursor $cursor, string $str, bool $bin, int $cut): void
    {
        ...
        if ('' === $str) {
            ...
            foreach ($strs as $s) {
                ...
                if (0 < $this->maxStringWidth && $this->maxStringWidth < $len = mb_strlen($s, 'UTF-8')) {
                    $s = mb_substr($s, 0, $this->maxStringWidth, 'UTF-8');
                ...
                if ('' !== $s) {
                    $this->line .= $this->style('str', $s, $attr);
                }
                if ($i++ == $m) {
                    if ($m) {
                        if ('' !== $s) {
    ...
    protected function dumpKey(Cursor $cursor): void
    {
        if (null !== $key = $cursor->hashKey) {
            ...
            $style = 'key';
            $hashType = $cursor->hashType;

            if (Cursor::HASH_INDEXED === $hashType) {
            ...
            } elseif (Cursor::HASH_ASSOC === $hashType) {
                if (\is_int($key)) {
                    $this->line .= $this->style($style, $key).' => ';
                } else {
                    $this->line .= $bin.'"'.$this->style($style, $key).'" => ';
                ...
                if (Cursor::HASH_RESOURCE === $hashType) {
                    $key = "\0~\0".$key;
                }
    ...
    protected function getSourceLink(string $file, int $line): string|false
    {
