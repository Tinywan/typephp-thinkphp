...
class ExceptionCaster
{
    ...
    public static function castTraceStub(TraceStub $trace, array $a, Stub $stub, bool $isNested): array
    {
        ...
        for ($j += $trace->numberingOffset - $i++; isset($frames[$i]); ++$i, --$j) {
            ...
            if (isset($f[$prefix.'src'])) {
                foreach ($f[$prefix.'src']->value as $label => $frame2) {
                ...
                $f = $frames[$i - 1];
                if ($trace->keepArgs && !empty($f['args']) && $frame2 instanceof EnumStub) {
                    $frame2->value['arguments'] = new ArgsStub($f['args'], $f['function'] ?? null, $f['class'] ?? null);
                }
            } elseif ('???' !== $lastCall) {
            ...
            $a[substr_replace($label, \sprintf('separator=%s&', $frame2 instanceof EnumStub ? ' ' : ':'), 2, 0)] = $frame2;
    ...
    public static function castFrameStub(FrameStub $frame, array $a, Stub $stub, bool $isNested): array
    {
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                $ellipsisLen = $ellipsis->attr['ellipsis'] ?? 0;
                ...
                $srcAttr .= $ellipsisLen ? '&ellipsis-type=path&ellipsis='.$ellipsisLen.'&ellipsis-tail='.$ellipsisTail : '';
    ...
    private static function extractSource(string $srcLines, int $line, int $srcContext, string $lang, ?string $file, array $frame): EnumStub
    {
        ...
        if ($frame['function'] ?? false) {
            ...
            $srcLinesArr = ["\0~separator=\0" => $stub];
        } else {
            $stub = null;
            $srcLinesArr = [];
        }
        ...
        foreach ($src as $i => $c) {
            ...
            $c->attr['lang'] = $lang;
            $srcLinesArr[\sprintf("\0~separator=› &%d\0", $i + $line - $srcContext)] = $c;
        }

        return new EnumStub($srcLinesArr);
    }
}
